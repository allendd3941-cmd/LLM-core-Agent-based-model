...
- **事件車**：鮮明**狀態色彩點**（移動藍 / 等紅燈琥珀 / 已抵達綠 / 異常灰），車種以大小分（汽車大、機車小），可點選 inspect。
- **背景車**：低調**灰色半透明小點**、不可點選——一眼就能和事件車區分。
- 圖例新增「背景常態車流」一條 + 「背景車流」顯示/隱藏開關；狀態列新增「背景車 N」。
- 車輛一律用畫布彩點（前端重設計後已移除舊的 emoji 圖標，見 `DEMO_FEATURES_zh-TW.md §18`），依 zoom 微幅放大；背景車再多也不影響事件車可讀性。
...
> 兩個不改結果的加速旗標（見 `docs/SCALING_zh-TW.md §6`）：`[scaling].cache_network`（跨連線/重設快取已解析路網+索引，
> 第 2 次起省掉 24MB 重解析）、`[scaling].init_workers>1`（車多時用 multiprocessing 並行算 init 路徑，Linux 受益最大）。
