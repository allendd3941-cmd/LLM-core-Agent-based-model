...
from llm_abm_simulator.config import ACTIVE_MODES, VEHICLE_TYPES, DEFAULT_CONFIG
...
def test_initialize_deterministic_same_seed():
    ...
    assert {k: (v.origin_town, v.vehicle_type, v.active_mode) for k, v in a1.items()} == \
           {k: (v.origin_town, v.vehicle_type, v.active_mode) for k, v in a2.items()}
...
def test_initialize_fields_valid():
    ...
    for asg in p.initialize_agents(_agents(), TOWNS).values():
        ...
        assert asg.active_mode in ACTIVE_MODES
...
def test_decide_step_rules():
    ...
    assert d[a.agent_id].active_mode == "avoid_congestion"
    a.congestion_proxy = 0.0
    a.distance_to_destination = 1000
    d = p.decide_step(agents, {}, 1)
    assert d[a.agent_id].active_mode == "short_distance"
