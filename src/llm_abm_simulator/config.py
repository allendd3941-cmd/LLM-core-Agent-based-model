...
class SimulationConfig:
    ...
    # 距離抵達（UXsim）：事件車一踏進抵達圈（對球場中心 ≤ arrival_radius_m）即算抵達，並把該車安全移出路網
    # （=停在球場周邊、離開車流）→ 終點前不再堆「排到節點」的 sink 隊伍。agent 仍留在畫面(凍在踏入點、顯示已抵達)。
    # False = 回退「走到終點節點才算抵達」（會在終點前回堵）。詳見 docs。
    arrival_on_circle_entry: bool = True
